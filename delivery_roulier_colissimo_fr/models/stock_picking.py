...
class StockPicking(models.Model):
    ...
    colissimo_insurance = fields.Selection(
        selection=[
            ("15000", "Assurance 150 €"),
            ("30000", "Assurance 300 €"),
            ("45000", "Assurance 450 €"),
    ...
    colissimo_recommande = fields.Selection(
        selection=[
    ...
    @api.constrains("colissimo_recommande", "colissimo_insurance")
    def _colissimo_fr_check_insurance(self):
        for rec in self:
            if rec.colissimo_recommande and rec.colissimo_insurance:
                raise (
                    "Les paramètres 'recommandé' et 'assurance' " "sont incompatibles."
                )
            if rec.colissimo_recommande and rec.partner_id.country_id != self.env.ref(
                "base.fr"
            ):
                message = (
    ...
    def _colissimo_fr_get_shipping_date(self, package_id=None):
    ...
    @api.model
    def _colissimo_fr_map_options(self):
        return {
            "NM": "nonMachinable",
            "FCR": "ftd",
            "COD": "cod",
    ...
    def _colissimo_fr_get_options(self, package):
        ...
        if "insuranceValue" in options:
            if self.colissimo_recommande:
                options["recommendationLevel"] = self.colissimo_recommande
                del options["insuranceValue"]
            else:
                options["insuranceValue"] = int(self.colissimo_insurance)
        if "cod" in options:
            options["codAmount"] = 0
        return options
    ...
    # helpers
    @api.model
    def _colissimo_fr_convert_address(self, partner):
