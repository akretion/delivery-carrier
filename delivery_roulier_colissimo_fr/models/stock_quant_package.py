...
class StockQuantPackage(models.Model):
    ...
    def _colissimo_fr_get_parcel(self, picking):
        ...
        vals.update(picking._colissimo_fr_get_options(self))
        if vals.get("COD"):
    ...
    def _colissimo_fr_get_customs(self, picking):
    ...
    def _colissimo_fr_should_include_customs(self, picking):
        ...
        # see https://boutique.colissimo.fr/_ui/doc/formalites_douane.pdf
    ...
    @api.model
    def _colissimo_fr_invalid_api_input_handling(self, payload, exception):
    ...
    def _colissimo_fr_carrier_error_handling(self, payload, exception):
        response = exception.response
        request = response.request

        # Clean sensitive data
        if "password" in payload["auth"]:
            payload["auth"]["password"] = "****"
        if "apiKey" in payload["auth"]:
            payload["auth"]["apiKey"] = "****"
        ...
        ret_mess = _(
            ...
            "Données transmises:\n"
            "-----------------------------\n%s"
        ) % ("\n".join(parts), request.body.decode("utf-8"))
        return ret_mess

    def _colissimo_fr_get_tracking_link(self):
        return (
